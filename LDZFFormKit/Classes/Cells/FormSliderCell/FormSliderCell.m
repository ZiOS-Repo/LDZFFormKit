...
@property(nonatomic, strong) FormSliderBean *dataItem;
...
- (void)loadContent {
    [super loadContent];
    if (![self.data isKindOfClass:[FormSliderBean class]]) return;
    ...
    self.arrowImg.image = self.dataItem.expend ? [FormManager imageName:@"arrow_up"] : [FormManager imageName:@"arrow_down"];
...
-(void)sliderValueDidChanged:(UISlider *)sender{
    if ([self.formDelegate respondsToSelector:@selector(sliderCell:bean:sliderValueDidChanged:)]) {
        [self.formDelegate sliderCell:self bean:self.dataItem sliderValueDidChanged:sender];
    }
}
...
-(void)sliderEnd:(UISlider *)sender{
    if ([self.formDelegate respondsToSelector:@selector(sliderCell:bean:sliderEnd:)]) {
        [self.formDelegate sliderCell:self bean:self.dataItem sliderEnd:sender];
    }
}

- (void)clickTapAreaBtn:(UIButton *)sender {
    if ([self.formDelegate respondsToSelector:@selector(sliderCell:bean:clickAreaBtn:)]) {
        [self.formDelegate sliderCell:self bean:self.dataItem clickAreaBtn:sender];
    }
}
...
- (void)actionTapGesture:(UITapGestureRecognizer *)sender {
    ...
    if ([self.formDelegate respondsToSelector:@selector(sliderCell:bean:sliderValueDidChanged:)]) {
        [self.formDelegate sliderCell:self bean:self.dataItem sliderValueDidChanged:self.valSlider];
    }
}
...
- (UIImageView *)arrowImg {
    if (!_arrowImg) {
        ...
        _arrowImg.image = [FormManager imageName:@"arrow_down"];
    }
    return _arrowImg;
}
