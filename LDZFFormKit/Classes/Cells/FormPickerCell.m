...
@property(nonatomic, strong) FormPickerCellModel *dataItem;
@end
...
- (void)loadContent {
    [super loadContent];
    if ([self.data isKindOfClass:[FormPickerCellModel class]]) {
        self.dataItem = self.data;
        self.extrImgView.image = self.dataItem.extrImg;
        if (FORM_STRING_ISEMPTY(self.dataItem.val)) {
...
- (void)clickTapAreaBtn:(UIButton *)sender {
    [kFormConfig.window endEditing:YES];
    if (self.delegate && [self.delegate respondsToSelector:@selector(formCell:event:)]) {
        [self.delegate formCell:self event:nil];
    }
}
