...
@property(nonatomic, strong) FormBtnsBean *dataItem;
@end
...
- (void)loadContent {
    [super loadContent];
    if (![self.data isKindOfClass:[FormBtnsBean class]]) return;
...
- (void)clickItem:(UIButton *)sender {
    ...
    [self refreshSelect:self.dataItem.val];
    if (self.formDelegate && [self.formDelegate respondsToSelector:@selector(formBtnsCellOfValueChanged:bean:)]) {
        [self.formDelegate formBtnsCellOfValueChanged:self bean:self.dataItem];
    }
}
