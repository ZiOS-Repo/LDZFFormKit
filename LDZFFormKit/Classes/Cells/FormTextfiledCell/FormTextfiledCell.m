...
@property(nonatomic, strong) FormTextfiledBean *dataItem;
@end
...
- (void)layoutSubviews {
    ...
    if ([self.formDelegate respondsToSelector:@selector(rightViewForTextField:bean:cell:)]) {
        self.rightView = [self.formDelegate rightViewForTextField:self.valTextfiled bean:self.dataItem cell:self];
        if (self.rightView) {
...
- (void)loadContent {
    [super loadContent];
    if (![self.data isKindOfClass:[FormTextfiledBean class]]) return;
...
- (void)textFieldDidChange:(UITextField *)textField {
    if ([self.formDelegate respondsToSelector:@selector(textfiledCell:bean:textFieldDidChange:)]) {
        [self.formDelegate textfiledCell:self bean:self.dataItem textFieldDidChange:textField];
    }
}

#pragma mark - UITextFieldDelegate
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    if ( [self.formDelegate respondsToSelector:@selector(textfiledCell:bean:textField:shouldChangeCharactersInRange:replacementString:)]) {
        return [self.formDelegate textfiledCell:self bean:self.dataItem textField:textField shouldChangeCharactersInRange:range replacementString:string];
    }
    return YES;
}

- (void)textFieldDidEndEditing:(UITextField *)textField {
    if ([self.formDelegate respondsToSelector:@selector(textfiledCell:bean:textFieldDidEndEditing:)]) {
        [self.formDelegate textfiledCell:self bean:self.dataItem textFieldDidEndEditing:textField];
    }
}
