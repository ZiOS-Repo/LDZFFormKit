...
- (nullable UIImage *)ldzf_imageByRotate:(CGFloat)radians fitSize:(BOOL)fitSize;
...
/// @param data gif 数据
/// @param scale 比例
+ (nullable UIImage *)ldzf_imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale;
...
+ (nullable UIImage *)ldzf_imageWithPDF:(id)dataOrPath;

/// 从PDF文件数据或路径创建图像。如果PDF有多页，则仅返回第一页的内容。 图像的比例等于当前屏幕的比例，尺寸与PDF的原始尺寸相同。
/// @param dataOrPath 数据或者路径
/// @param size 尺寸
+ (nullable UIImage *)ldzf_imageWithPDF:(id)dataOrPath size:(CGSize)size;
